# TODO – StageTrack

> Cochez les cases au fur et à mesure. Synchronisez avec votre outil de gestion (Trello/Notion).

---

## Sprint 0 – Setup & Organisation

- [x] Créer le dépôt GitLab et inviter les membres
- [x] Définir la stack technique finale (framework, BDD)
- [x] Rédiger le `.gitignore` adapté au stack
- [x] Créer les branches `main` et `develop`
- [x] Préparer l'environnement local (WAMP / XAMPP / Docker)
- [x] Créer le fichier `.env.example`
- [x] Réaliser le diagramme de Gantt initial
- [-] Réaliser les wireframes des pages principales

---

## Sprint 1 – Base de données & Architecture

- [ ] Concevoir le schéma MCD / MLD
- [ ] Créer les migrations : `users`, `roles`, `stages`, `offres`, `documents`
- [ ] Créer les seeders avec données de test
- [ ] Mettre en place le système de rôles (admin, étudiant, tuteur, entreprise, jury)
- [ ] Configurer la connexion BDD dans `.env`

---

## Sprint 2 – Authentification

- [ ] Page de connexion (formulaire + validation)
- [ ] Authentification à deux niveaux (mot de passe + vérification email ou 2FA)
- [ ] Redirection selon le rôle après connexion
- [ ] Déconnexion / gestion de session
- [ ] Inscription entreprise (avec validation admin)
- [ ] Réinitialisation de mot de passe

---

## Sprint 3 – Espace Administrateur

- [ ] Dashboard admin (statistiques, flux de requêtes)
- [ ] Gestion des utilisateurs (CRUD)
- [ ] Validation des formations / filières demandées
- [ ] Gestion des offres de stage
- [ ] Notifications périodiques des requêtes étudiantes
- [ ] Fichier trace des demandes/avis

---

## Sprint 4 – Espace Étudiant

- [ ] Dashboard étudiant
- [ ] Consultation et recherche des offres (filière, durée, missions)
- [ ] Suivi des étapes de validation du dossier
- [ ] Dépôt de documents (rapport, résumé, fiche d'évaluation)
- [ ] Cahier de stage (journal de bord)
- [ ] Ajout de remarques
- [ ] Envoi de requête pour ajouter une formation manquante

---

## Sprint 5 – Espace Entreprise

- [ ] Dashboard entreprise
- [ ] Dépôt d'offre de stage
- [ ] Validation de la convention en ligne
- [ ] Consultation des candidatures étudiantes
- [ ] Attribution de missions aux étudiants
- [ ] Ajout de remarques

---

## Sprint 6 – Espace Tuteur

- [ ] Dashboard tuteur
- [ ] Validation des conventions de stage
- [ ] Suivi de l'avancement des étudiants
- [ ] Téléchargement des documents de restitution
- [ ] Affectation de stages
- [ ] Dépôt de rapport
- [ ] Ajout de remarques

---

## Sprint 7 – Espace Jury

- [ ] Dashboard jury
- [ ] Consultation des documents et cahiers de stage
- [ ] Consultation des tâches affectées
- [ ] Validation des stages
- [ ] Ajout de remarques sur le rapport

---

## Sprint 8 – UI / UX & Responsive

- [ ] Design cohérent sur toutes les pages
- [ ] Responsive mobile (Mobile First)
- [ ] Accessibilité WCAG (alt images, contrastes, balises sémantiques)
- [ ] Optimisation performance (lazy loading, compression images, cache)
- [ ] Méta-descriptions et titres SEO

---

## Sprint 9 – Tests & Finalisation

- [ ] Tests sur Chrome, Firefox, Safari
- [ ] Tests mobile, tablette, desktop
- [ ] Tests de performance
- [ ] Relecture du code et refacto
- [ ] Rédaction du rapport (15 pages max)
- [ ] Préparation de la soutenance (20 min max)
- [ ] Merge final `develop` → `main`

---

## Notes

...
const ACCENT = { bg: '#FFFBEB', text: '#D97706', border: '#FDE68A', stripe: '#FEF3C7' };

const NAV_ITEMS = [
    { label: 'Tableau de bord',  href: 'entreprise.dashboard',    icon: HomeIcon },
    { label: 'Mes offres',       href: 'entreprise.index.offre',  icon: ListIcon },
    { label: 'Candidatures',     href: 'entreprise.candidatures', icon: InboxIcon },
    { label: 'Mes stages',       href: 'entreprise.index.stage',  icon: BriefcaseIcon },
        ...
        <div style={{ minHeight: '100vh', display: 'flex', background: 'var(--bg-page)', fontFamily: "'Inter', sans-serif" }}>
            ...
            <aside className="ds-sidebar" style={{ width: sidebarOpen ? 240 : 64 }}>
                ...
                <div style={{ height: 60, display: 'flex', alignItems: 'center', padding: '0 16px', borderBottom: '1px solid var(--border)', gap: 12, flexShrink: 0 }}>
                    {sidebarOpen && (
                        <Link href={route('entreprise.dashboard')} className="ds-logo" style={{ flex: 1, color: ACCENT.text }}>
                    ...
                    {NAV_ITEMS.map(item => {
                        ...
                        return (
                            <Link key={item.href} href={route(item.href)}
                                className={`ds-nav-item ${isActive ? 'active amber' : ''}`}
                ...
                <div style={{ padding: '12px 8px', borderTop: '1px solid var(--border)', display: 'flex', flexDirection: 'column', gap: 4 }}>
                    {sidebarOpen && (
                        <div style={{ padding: '8px 12px', borderRadius: 8, background: 'var(--bg-page)', marginBottom: 4 }}>
                            <div style={{ fontSize: 13, fontWeight: 600, color: 'var(--text-primary)', lineHeight: 1.3 }}>{user?.nom}</div>
